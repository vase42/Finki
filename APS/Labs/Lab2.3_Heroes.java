...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        SLLNode<Card> naj = firstFriendCards.getFirst();
        SLLNode<Card> sec = secondFriendCards.getFirst();
        sec = sec.succ.succ;
        Card theOne = naj.element;
        naj = naj.succ;
        for(int i=1;i<6;i++){
            if(naj.element.damage()>theOne.damage()){
                theOne = naj.element;
            }
            naj = naj.succ;
        }
        firstFriendCards.delete(firstFriendCards.find(theOne));
        secondFriendCards.insertAfter(theOne,sec);
    }
