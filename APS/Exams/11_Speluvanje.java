...
public class Speluvanje {
    public static void main(String[] args) throws IOException {
         ...
         */
        for(int i=0;i<N;i++){
            Zbor zbor = new Zbor(br.readLine());
            if(tabela.search(zbor)==-1) tabela.insert(zbor,"");
        }
        List<String> lista = new ArrayList<>();
        String [] text = br.readLine().split("\\s+");
        if(text.length==1&&!Character.isAlphabetic(text[0].charAt(0))){
        ...
        for(String s : text){
            ...
            g = g.toLowerCase();
            Zbor zbro4e = new Zbor(g);
            if(tabela.search(zbro4e)==-1){
                lista.add(g);
            }
        }
        if(lista.size()==0) System.out.println("Bravo");
        else {
